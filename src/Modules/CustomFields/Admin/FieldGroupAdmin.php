...
<script type="text/html" id="tmpl-cc-rule-group">
    <div class="cc-rule-group">
        <div class="cc-rule-group-header">
            ...
            <span class="group-logic"><?php esc_html_e('OR', 'content-core'); ?></span>
        </div>
        <div class="cc-rule-group-content">
            <div class="cc-rules-container"></div>
            <div class="cc-builder-actions">
                <button type="button" class="button button-small cc-add-rule-btn" data-cc-action="add-rule"><?php esc_html_e('+ Add Rule', 'content-core'); ?></button>
                <button type="button" class="button button-small cc-remove-rule-group-btn" data-cc-action="remove-rule-group" style="color: #d63638;"><?php esc_html_e('Remove Group', 'content-core'); ?></button>
            </div>
        </div>
    </div>
</script>
...
<script type="text/html" id="tmpl-cc-rule-row">
    <div class="cc-rule-row">
        <div class="rule-logic">{{data.isFirst ? '' : 'AND'}}</div>
        ...
        <button type="button" class="cc-remove-rule-btn cc-remove-btn-subtle" data-cc-action="remove-rule" title="<?php esc_attr_e('Remove Rule', 'content-core'); ?>">&times;</button>
    </div>
</script>
...
<?php
	...
	public function save_field_group(int $post_id, \WP_Post $post): void
	{
		...
		if (!wp_verify_nonce($_POST['cc_assignment_nonce'], 'save_cc_assignment') ||
...
<script>
	jQuery(document).ready(function ($) {
		...
		assignmentContainer.on('click', '.cc-add-rule-btn', function () {
			var gIdx = $(this).closest('.cc-rule-group').data('index');
			if (!ruleGroups[gIdx].rules) ruleGroups[gIdx].rules = [];
			ruleGroups[gIdx].rules.push({ type: 'post_type', value: 'post' });
			renderAssignments();
		});

		assignmentContainer.on('click', '.cc-remove-rule-btn', function () {
			$(this).closest('.cc-rule-row').remove();
			updateHiddenAssignments();
		});
